...
class DatasetCatalog(object):
    ...
    (which are names that identify a datasets, e.g. "coco_2014_train")
    to a function which parses the datasets and returns the samples in the
    format of `list[dict]`.
    ...
    def register(name, func):
        """
        Args:
            name (str): the name that identifies a datasets, e.g. "coco_2014_train".
    ...
    def get(name):
        ...
        Returns:
            list[dict]: datasets annotations.0
        """
        try:
...
class Metadata(types.SimpleNamespace):
    ...
    It is intended for storing metadata of a datasets and make it accessible globally.
...
class MetadataCatalog:
    """
    MetadataCatalog provides access to "Metadata" of a given datasets.
    ...
    def get(name):
        """
        Args:
            name (str): name of a datasets (e.g. coco_2014_train).
...
metadata to each split (now called datasets) separately!
                    """
                )
