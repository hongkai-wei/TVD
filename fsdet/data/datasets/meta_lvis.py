...
def load_filtered_lvis_json(json_file, image_root, metadata, dataset_name=None):
    ...
    Args:
        ...
        dataset_name (str): the name of the datasets (e.g., "lvis_v0.5_train").
            If provided, this function will put "thing_classes" into the metadata
            associated with this datasets.
    ...
    if dataset_name is not None and 'train' in dataset_name:
        assert global_cfg.MODEL.ROI_HEADS.NUM_CLASSES == len(metadata['thing_classes']), \
...
def register_meta_lvis(name, metadata, json_file, image_root):
    """
    Register a datasets in LVIS's json annotation format for instance detection.

    Args:
        name (str): a name that identifies the datasets, e.g. "lvis_v0.5_train".
        metadata (dict): extra metadata associated with this datasets.
            It can be an empty dict.
        json_file (str): path to the json instance annotation file.
        image_root (str): directory which contains all the images.
    """
    DatasetCatalog.register(
        name, lambda: load_filtered_lvis_json(
            json_file, image_root, metadata, name)
    )

    MetadataCatalog.get(name).set(
        json_file=json_file, image_root=image_root, evaluator_type="lvis",
        **metadata,
    )
