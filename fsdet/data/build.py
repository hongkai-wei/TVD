...
def filter_images_with_only_crowd_annotations(dataset_dicts):
    ...
    A common training-time preprocessing on COCO datasets.
...
def print_instances_class_histogram(dataset_dicts, class_names):
    """
    Args:
        dataset_dicts (list[dict]): list of datasets dicts.
...
def build_batch_data_sampler(
    ...
    """
    Return a datasets index sampler that batches datasets indices possibly with
    ...
    Args:
        ...
        grouping_features (None, list[number], tuple[number]): If None, then grouping
            ...
            in the underlying datasets the value to be used for placing that datasets
...
def get_detection_dataset_dicts(dataset_names, filter_empty=True):
    """
    Load and prepare datasets dicts for instance detection.

    Args:
        dataset_names (list[str]): a list of datasets names
    ...
    if has_instances:
        ...
        except AttributeError:  # class names are not available for this datasets
            pass
    return dataset_dicts
...
def build_detection_train_loader(cfg, mapper=None):
    ...
    Args:
        ...
        mapper (callable): a callable which takes a sample (dict) from datasets and
...
def build_detection_test_loader(cfg, dataset_name, mapper=None):
    ...
    Args:
        cfg: a fsdet CfgNode
        dataset_name (str): a name of the datasets that's available in the DatasetCatalog
        mapper (callable): a callable which takes a sample (dict) from datasets
