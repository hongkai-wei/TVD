...
def transform_proposals(dataset_dict, image_shape, transforms, min_box_side_len, proposal_topk):
    ...
    Args:
        dataset_dict (dict): a dict read from the datasets, possibly
...
def annotations_to_instances(annos, image_size):
    ...
    from instance annotations in the datasets dict.
...
def check_metadata_consistency(key, dataset_names):
    ...
    for idx, entry in enumerate(entries_per_dataset):
        if entry != entries_per_dataset[0]:
            logger.error(
                "Metadata '{}' for datasets '{}' is '{}'".format(key, dataset_names[idx], str(entry))
            )
            logger.error(
                "Metadata '{}' for datasets '{}' is '{}'".format(
