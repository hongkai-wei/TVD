...
_C.INPUT.FORMAT = "BGR"
# peroid to visualize top-scoreing proposals from RPN.
...
# -----------------------------------------------------------------------------
_C.DATASETS = CN()
# List of the datasets names for training. Must be registered in DatasetCatalog
_C.DATASETS.TRAIN = ()
# List of the datasets names for testing. Must be registered in DatasetCatalog
_C.DATASETS.TEST = ()
...
# choose from "build_resnet_backbone" and "build_dla_backbone"
_C.MODEL.BACKBONE.NAME = "build_resnet_backbone"
# There are 5 stages in ResNet. The first is a convolution(stem), and the following
# stages are each group of residual blocks (res1, res2, res3, res4)
...
# to unfreeze the second 1024x1024 fc
_C.MODEL.ROI_HEADS.UNFREEZE_FC2 = False
# to unfreeze the first 12504x1024 fc
...
# ------------------------------
# perform Supervised Contrastive Loss w/ MoCo queue
# ------------------------------

# MoCo Supervised Contrastive Learning
_C.MODEL.MOCO = CN({'ENABLED': False})
# 单看 all_gather 没有 process group， 这里设置成 True 进行单卡 debug
_C.MODEL.MOCO.DEBUG_DEQUE_AND_ENQUE = False
# the number of iters to save MoCo queue, 0 to not save.
...
# MoCo loss weight is scaled linearly with training iters. when iters reach WARM_UP_STEPS, weight = 1
_C.MODEL.MOCO.WARM_UP_STEPS = 200
# Training MoCo Queue can be difficult, so one might need to adjust the loss weight.
...
# See fsdet/solver/build.py for LR scheduler options
...
# Mute the output of the environment info, full config, and model definition
...
#
# Do not commit any configurations into it.
_C.GLOBAL = CN()
_C.GLOBAL.HACK = 1.0


################################################################################
# Unused parameters in Detectron2
################################################################################
...
# The number of cascade stages is implicitly defined by the length of the following two configurations.
...
# currently albumentation is buggy, involves following:
    # dataset_mapper.py
    # train_net.py build_train_loader(cls, cfg)
# albumentation augmentation https://albumentations.ai/docs/examples/serialization/#using-yaml-insted-of-json
