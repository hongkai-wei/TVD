...
def get_checkpoint_url(config_path):
    ...
    Args:
        config_path (str): config file name relative to FsDet's "configurations/"
...
def get(config_path, trained: bool = False):
    """
    Get a model specified by relative path under FsDet's official ``configurations/`` directory.
    Args:
        config_path (str): config file name relative to FsDet's "configurations/"
            directory, e.g., "COCO-detection/faster_rcnn_R_101_FPN_ft_all_1shot.yaml"
